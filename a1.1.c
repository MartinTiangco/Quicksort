...
/* Quick sort on the thread */
void *quick_sort_thread(void *args)
{
    struct block *actual_data = args;

    quick_sort(*actual_data);
}

/* On first call of quick_sort, will create the thread for the right side */
void quick_sort_helper(struct block my_data)
{
    pthread_t thread;
    ...
    // tries to create the second thread
    if (pthread_create(&thread, NULL, quick_sort_thread, (void *)&left_side))
    {
        exit(1);
    };
    quick_sort(right_side);
